...
cost1 = (0.5-alpha)*loss1
cost2 = (0.5+alpha)*loss2 + 5e-4 * regularizers
cost = cost1 + cost2

pred_op = tf.train.MomentumOptimizer(learning_rate,0.9).minimize(cost1,global_step=batch)
clus_op = tf.train.MomentumOptimizer(learning_rate,0.9).minimize(cost2,global_step=batch)
...
for i in range(1000):
    if i== 20:
        ...
        set_op = C.assign(sess.run(FC,feed_dict={X:trX[test_idx]})[:10])
    ...
    for start, end in zip(range(0, len(trX), nBatch), range(nBatch, len(trX), nBatch)):
        ...
        gt_ = trY[start:end]
        _,l2, p = sess.run([pred_op,cost2, pred], feed_dict={X: input_, Y: gt_, alpha:alpha_})
        _,l1  = sess.run([clus_op,cost1], feed_dict={X: input_, Y: gt_, alpha:alpha_})
        trainCost = l1+l2
        if cnt % 100 == 0: 
            print "Iter %d, L %5.2f, L1 %5.2f, L2 %5.2f, P %3.2f, time %.1f" % (cnt, trainCost, l1, l2, np.sum(np.argmax(p, 1) == gt_)/float(nBatch), time.time() - start_time)
